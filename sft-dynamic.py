...
from common import parse_refid, uint, uint8, uint16, uint32

def parse_uint(bits, i, data):
    return bits//8, uint(bits, data[i:i+4])
    #if bits == 32:
    #    return 4, uint32(data[i:i+4])
    #elif bits == 16:
    #    return 2, uint16(data[i:i+2])
    #elif bits == 8:
    #    return 1, uint8(data[i:i+1])

def parse_float32(i, data):
    return struct.unpack('f', data[i:i+4])[0]

def parse_wstring(i, data):
    length = uint16(data[i:i+2])
    return 2 + length, data[i+2:i+2+length].decode()

def parse_vsval(i, data):
    byte1 = data[i]
...
def parse_refids(refidcount, i, data):
    #TODO
    return refidcount*3, data[i:i+refidcount*3]

def parse_factions(factioncount, i, data):
    #TODO
    return factioncount*4, data[i:i+factioncount*4]

def parse_filetime(i, data):
    # TODO: this crap
    return data[i:i+8]

def parse_screenshot(w, h, i, data):
    return w*h*3, data[i:i+w*h*3]

def parse_plugininfo(i, data):
...
def parse_formidarray(formidarraycount, i, data):
    formids = []
    for _ in range(formidarraycount):
        formids.append(hex(uint32(data[i:i+4])))
        i += 4
    return i, formids

def find_player_changeform(cfcount, formidarray, i, data):
    """
        ...
        for line in instructions:
            ...
            if cmd == 'flag':
                ...
                if args[1].isdigit():
                    ...
                    count = pldict[args[1]]
                offset, pldict[args[0]] = parse_refids(count, i, data)
                i += offset
            elif cmd == 'refid':
                offset, pldict[args[0]] = parse_refids(1, i, data)
                i += offset
            elif cmd == 'factions':
                count = pldict[args[1]]//4
                offset, pldict[args[0]] = parse_factions(count, i, data)
                i += offset
            else:
                raise SyntaxError('Unknown command: {}'.format(cmd))
        if data[i:]:
